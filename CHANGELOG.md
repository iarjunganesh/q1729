...
- **P1-R4 first archived run under the committed protocol.** `benchmarks/runs/2026-09-06-1286200412954fb4a59cac02c27a06df.json` — schema 4, protocol digest `24d1d3af702a`, clean source `15c0da6`, `nvidia`/fp32, driver 616.56, `turbo`, 4000 shots, 27 rows, with run-specific figures. The 2026-08-05 archive is untouched and both validate side by side. Against it: the classical arm reproduces within ±12% (16384 terms 68.997 to 67.584 ms) with saturation unchanged at 16.00 digits; QAE timing shifts systematically with m (m=2 ~25% faster, m=15/16 ~7-10% slower), recorded as an observation because this run did not isolate driver change from thermal drift; and all 15 QAE rows reproduce the archive's pi estimate to 1e-12, agree with closed-form theory, and sit below their own sampling tolerance. **The findings document and its human review record are still open — agents do not approve on a reviewer's behalf, so P1-R4 is not closed and no tag is cut.**
...
- **P1-R3 complete: runtime restored and the classical arm profiled.** WSL2's virtual disk was found deleted (not detached); the distro was rebuilt as Ubuntu 26.04 with a uv-managed Python 3.13.15 venv, because 26.04 ships 3.14 and cudaq publishes no 3.14 wheel. On the restored RTX 5070 host the suite is **344 passed, 1 skipped, 100.00% coverage** — every module at 100%, the gate met for the first time. Profiling (reproduced twice) shows `kernel_handle`, the per-call `RawModule` construction, is a **constant ~3.1-3.9 ms** and accounts for ~92% of the two-term measurement, while device `execute` is ~1% (~30 microseconds). The archived 2.714 ms figure and the "sub-millisecond" claim were measuring different things; below roughly n = 1024 the classical arm's wall time is a measurement of Python, not of the GPU. No per-gate dispatch claim about the quantum arm is licensed — that arm is still unprofiled.
