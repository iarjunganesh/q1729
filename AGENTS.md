...
derives the QAE error floor analytically. GPU profiling from 2026-09-06 and
two archives exist; the GPU test suite was re-verified on 2026-09-23.
...
**One Python line everywhere: 3.14** (owner decision, 2026-09-23). Windows
`.venv`, the WSL2 venv, CI and `requires-python = ">=3.14"` all use it; Ruff
and mypy target it, and the source uses 3.14-only syntax (PEP 758 unparenthesized
`except A, B:`), so an older interpreter cannot even parse the package. Verified
2026-09-23: `cuda-quantum-cu13` 0.16.0.post1 publishes cp314 Linux wheels and
requires `cupy-cuda13x>=14.0.0` on 3.14; CuPy 14.2.0, NVRTC 13.4.92, numpy,
matplotlib and the rest publish cp314 (or pure-Python) wheels. Before moving to
a newer Python line, check CUDA-Q's published wheels and dependency metadata
first — it is always the last package to support a new interpreter. Installing
or replacing an interpreter on the owner's machine is a system-level change;
confirm with the owner before doing so. Every other pin should move without
that extra scrutiny.
